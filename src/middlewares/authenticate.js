...
module.exports = async (req, res, next) => {
    ...
    if (!bearerToken || !bearerToken.startsWith('Bearer ')) {
        return res.status(500).send({ message: "Please provide a valid token", status: "failed" });
    }
    ...
    if (!user) return res.status(500).send({ message: "please provide a valid token", status: "failed" });
